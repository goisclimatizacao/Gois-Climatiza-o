            ...
            <div className="grid grid-cols-7 gap-1">
                ...
                {dates.map(date => {
                    ...
                    return (
                        <div 
                            key={date.toString()} 
                            className={`border border-gray-200 dark:border-gray-700 rounded-lg min-h-[120px] p-2 flex flex-col ${isCurrentMonth ? 'bg-white dark:bg-gray-800' : 'bg-gray-50 dark:bg-gray-800/50'}`}
                        >
                            ...
                            <div className="mt-1 space-y-1 overflow-y-auto">
                                {posts.map(post => (
                                    <div 
                                        ...
                                        className="bg-green-50 dark:bg-green-900/30 border border-green-200 dark:border-green-800/50 p-1.5 rounded-md cursor-pointer hover:shadow-md hover:border-green-400 transition-all duration-200"
                                    >
                                        <div className="flex items-center space-x-2">
                                            <img src={post.content.selectedImageUrl} alt="Post image" className="w-8 h-8 rounded object-cover flex-shrink-0" />
                                            <div>
                                                {/* FIX: Access caption via post.content.post.content.caption */}
                                                <p className="text-xs text-gray-700 dark:text-gray-300 font-medium line-clamp-2">{post.content.post.content.caption}</p>
